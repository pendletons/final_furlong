defmodule FinalFurlongWeb.Authorize do
  @moduledoc """
  Logic for authorising users and guests
  """
  ...
  def guest_check(%Plug.Conn{assigns: %{current_user: _current_user}} = conn, _opts) do
    ...
    |> render(FinalFurlongWeb.AuthView, "logged_in.json", [])
    |> halt
  end
  ...
  def error(conn, status, code) do
    conn
    |> put_status(status)
    |> render(FinalFurlongWeb.AuthView, "#{code}.json", [])
    |> halt
  end
end
